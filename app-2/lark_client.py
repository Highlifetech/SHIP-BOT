...
def _end_column(cols, floor="R"):
    """The furthest column letter anyone has mapped, never left of `floor`.

    The read range used to be hard-coded to A:R, which quietly meant a column
    added at S could be mapped and then never read.
    """
    best = col_to_index(floor) or 0
    for letter in cols.values():
        idx = col_to_index(str(letter or ""))
...
class LarkClient:
    ...
    def read_tracking_data(self, spreadsheet_token, sheet_id):
        ...
        # Packing-list fields. Mapped in config since the beginning, never read
        # until now, because the bot only cared about where a parcel was.
        i_photo = col_to_index(cols.get("product_photo", ""))
        i_product = col_to_index(cols.get("product_name", ""))
        i_qty_shipped = col_to_index(cols.get("qty_shipped", ""))
        i_qty_expected = col_to_index(cols.get("qty_expected", ""))
        i_notes = col_to_index(cols.get("notes", ""))
        # The rest of the packing-list template. Unmapped until someone adds
        # the columns and sets the env vars, and blank until then.
        i_box = col_to_index(cols.get("box_num", ""))
        i_ship_to = col_to_index(cols.get("ship_to", ""))
        i_invoice = col_to_index(cols.get("invoice_num", ""))
        i_ship_date = col_to_index(cols.get("ship_date", ""))
        i_ddp = col_to_index(cols.get("ddp", ""))
        ...
        # Read out to the furthest column anyone has mapped, not a fixed R --
        # otherwise adding a column past R maps it and then never reads it.
        end_col = _end_column(cols)
        rows = self.read_sheet_range(
            spreadsheet_token, sheet_id,
            start_col="A", end_col=end_col,
        ...
        for i, row in enumerate(rows):
            ...
            results.append({
                ...
                "product_name": cell(row, i_product),
                "product_photo": cell(row, i_photo),
                "qty_shipped": cell(row, i_qty_shipped),
                "qty_expected": cell(row, i_qty_expected),
                "notes": cell(row, i_notes),
                "box_num": cell(row, i_box),
                "ship_to": cell(row, i_ship_to),
                "invoice_num": cell(row, i_invoice),
                "ship_date": cell(row, i_ship_date),
                "ddp": cell(row, i_ddp),
            })
    ...
    def read_order_rows(self, spreadsheet_token, sheet_id):
        """Every row that looks like an order line, tracking or not.

        read_tracking_data drops rows with no tracking number, which is right
        for a tracker and wrong for a shipping app: an order line that hasn't
        shipped yet is precisely the thing someone is about to pack. This
        keeps them, and carries the same carry-forward for client and order
        number so continuation lines know what they belong to.
        """
        cols = columns_for(spreadsheet_token)
        idx = {name: col_to_index(cols.get(name, ""))
               for name in ("order_num", "customer", "recipient", "tracking_num",
                            "product_name", "product_photo", "qty_shipped",
                            "qty_expected", "num_boxes", "status", "box_num")}
        ...
        start_row = header_row_for(spreadsheet_token) + 1
        rows = self.read_sheet_range(
            spreadsheet_token, sheet_id, start_col="A",
            end_col=_end_column(cols), start_row=start_row, end_row=500)

        out = []
        last_client = last_order = ""
        for i, row in enumerate(rows):
            if not isinstance(row, list) or not any(str(c or "").strip() for c in row):
                last_client = last_order = ""
                continue
            client = cell(row, "customer") or cell(row, "recipient") or last_client
            order = cell(row, "order_num") or last_order
            if cell(row, "customer") or cell(row, "recipient"):
                last_client = cell(row, "customer") or cell(row, "recipient")
            if cell(row, "order_num"):
                last_order = cell(row, "order_num")

            product = cell(row, "product_name")
            ordered = cell(row, "qty_expected")
            if not product and not ordered:
                continue
            out.append({
                "row_num": start_row + i,
                ...
                "product_photo": cell(row, "product_photo"),
                "qty_shipped": cell(row, "qty_shipped"),
                "qty_expected": ordered,
                "tracking_num": clean_tracking(cell(row, "tracking_num")),
                "num_boxes": cell(row, "num_boxes"),
                "box_num": cell(row, "box_num"),
                "current_status": cell(row, "status"),
            })
        logger.info("  %d order lines in sheet %s", len(out), sheet_id)
        return out
    ...
    def _send_card(self, message, chat_id, message_id=None, card_json=None):
        ...
        if not resp.ok:
            # Lark puts the useful part (which field it disliked) in the body,
            # and raise_for_status throws it away. Log it before raising.
            logger.error("Card rejected (HTTP %s): %s", resp.status_code,
                         resp.text[:600])
        resp.raise_for_status()
        data = resp.json()
        if data.get("code") != 0:
    ...
    def send_daily_summary(self, all_results, chat_id=None, message_id=None):
        ...
        # each one linted against the documented schema before it is sent so a
        # malformed card is caught here rather than by a bare 400 from Lark.
        import card_lint
        builders = []
        if os.environ.get("CARD_SCHEMA", "2.0").strip() == "2.0":
            builders.append(("2.0", card_builder.build_tracker_card_v2))
        builders.append(("1.0", card_builder.build_tracker_card))
        for schema, build in builders:
            try:
            ...
            if problems:
                logger.error("%s produced an invalid card, skipping: %s",
                             build.__name__, "; ".join(problems[:4]))
                continue
            try:
                self._send_card(None, target_chat, message_id=message_id,
                                card_json=json.dumps(card))
                logger.info("Daily summary sent (schema %s)", schema)
                return
            except Exception as e:
                logger.warning("Send failed on schema %s (%s), trying next",
                               schema, e)
