...
# ------------------------------------------------- hand-typed lines
# A shipment the team enters from scratch has no order line behind it: no key
# in `available`, no ordered figure, and possibly its own client.
MANUAL = dict(GOOD)
MANUAL["lines"] = [{"key": "manual:1", "product": "Sample pack",
                    "order_num": "SO-NEW-1", "client": "Farmer Boys",
                    "ordered": 25, "qty": 25, "box": 1}]
MANUAL["boxes"] = 1
check("a hand-typed line validates", sc.validate(MANUAL, avail) == [])
mwork = sc.plan(MANUAL, ROWS, COLUMNS)
mcells = {(u["col"], u["row"]): u["value"] for u in mwork["updates"]}
check("a hand-typed line keeps its own client",
      mcells.get(("E", 9)) == "Farmer Boys")
check("a hand-typed line keeps its sales order",
      mcells.get(("D", 9)) == "SO-NEW-1")
check("a hand-typed line writes its quantities",
      mcells.get(("J", 9)) == "25" and mcells.get(("K", 9)) == "25")
check("a shipment with no board is caught",
      any("which board" in x for x in sc.validate(dict(MANUAL, **{"tab": ""}), avail)))
check("a hand-typed line is not capped by open quantity",
      sc.validate(dict(MANUAL, **{"lines": [dict(MANUAL["lines"][0], **{"qty": 9999})]}),
                  avail) == [])

# The shipment's client still applies to picked lines that don't name one.
check("picked lines still take the shipment client",
      {(u["col"], u["row"]): u["value"]
