...
COLUMNS = {
    ...
    "notes": "Q",
    "delivery_date": "R",
    # Packing lists need a product name. The main inbound sheets don't have a
    # column for one yet -- when Hannah adds it, point at it with
    # LARK_PRODUCT_NAME_COL (e.g. "S") rather than editing this file.
    "product_name": os.environ.get("LARK_PRODUCT_NAME_COL", "").strip().upper(),
    # The rest of Off Menu's packing-list template. None of these have a
    # column on the inbound sheets yet, so they read and write as blank and
    # the packing list flags them. Add the columns, point these at them, and
    # the flags go away -- no code change.
    #
    #   box_num      which box a line went in (what the per-box lists need)
    #   ship_to      the SHIPPING ADDRESS block
    #   invoice_num  Invoice #
    #   ship_date    Date Shipped
    #   ddp          the DDP cell
    "box_num": os.environ.get("LARK_BOX_NUM_COL", "").strip().upper(),
    "ship_to": os.environ.get("LARK_SHIP_TO_COL", "").strip().upper(),
    "invoice_num": os.environ.get("LARK_INVOICE_COL", "").strip().upper(),
    "ship_date": os.environ.get("LARK_SHIP_DATE_COL", "").strip().upper(),
    "ddp": os.environ.get("LARK_DDP_COL", "").strip().upper(),
}
...
# Notes columns.
# The per-client sheets already carry a Product Names column (D). It was
# ignored while the bot only cared about tracking; a packing list needs it.
